...
ax2.errorbar(range(len(mean_str_og)),mean_str_og,yerr=[float(x)/2 for x in sd_str_og],capsize=3,label="Strain contigs",color='red')
ax2.errorbar(range(len(mean_con_og)),mean_con_og,yerr=[float(x)/2 for x in sd_con_og],capsize=3,label="Consensus contigs",color='blue')
ax2.legend(loc='lower left')
ax2.set_title("Average genome fraction for all genomes")
ax4.errorbar(range(len(mean_str_ev)),mean_str_ev,yerr=[float(x)/2 for x in sd_str_ev],capsize=3,label="Strain contigs",color='red')
ax4.errorbar(range(len(mean_con_ev)),mean_con_ev,yerr=[float(x)/2 for x in sd_con_ev],capsize=3,label="Consensus contigs",color='blue')
...
f.text(0.08,0.5,"Genome fraction (%)",ha = 'center', va = 'center', rotation='vertical')
f.text(0.5,0.05,"Genome similarity (ANI)",ha = 'center', va = 'center')
plt.show()
